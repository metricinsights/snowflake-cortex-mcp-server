...
async def ask(
    ...
    for i in result:
        if i.event_type == "response.text.delta":
            ...
            await ctx.report_progress(progress=1, total=3, message=i.data.get('message', i.data['status']))
            ...
            columns = i.data.get('result_set', {}).get('resultSetMetaData', {}).get('rowType', [])
            for column in columns:
                out_table['columns'].append({
                    'key': column['name'],
                    'label': column['name'].replace('_', ' ').replace('-', ' ').title()
                })
